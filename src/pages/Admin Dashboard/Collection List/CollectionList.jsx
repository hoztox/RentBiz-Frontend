    ...
    <div className="w-full rounded-md border border-[#E9E9E9] collection-list-table mb-[80px] md:mb-0">
      ...
      <div className="flex justify-between border-b border-[#E9E9E9] collection-list-tabs">
        <div className='px-5 pb-5 flex flex-wrap gap-[10px]'>
          {['Paid', 'Unpaid', 'Overdue', 'All'].map((filter) => (
            <button
              key={filter}
              className={`w-[80px] text-center px-4 py-2 rounded-[4px] duration-150 filter-btns ${
                activeFilter === filter ? `${filter.toLowerCase()}-active` : `${filter.toLowerCase()}-inactive`
              }`}
        ...
        <div className="flex mx-5 gap-[13.36px] inputs-drop">
          ...
          <div className="relative w-[121px]">
            ...
            >
              {selectedOption}
              <ChevronDown className={`w-4 h-4 ml-2 transition-transform duration-300 ${isDropdownOpen ? "rotate-180" : "rotate-0"}`} />
            </button>
              ...
              <React.Fragment key={`${item.id}-${index}`}>
                <tr
                  className={`lg:hover:bg-gray-50 cursor-pointer h-[57px] ${
                    activeDropdowns[item.id] ? 'mobile-no-border' : 'border-b border-[#E9E9E9]'
                  }`}
                >
                  <td className="px-5 collection-list-data collection-list-id">{item.id}</td>
                  <td className="px-5 collection-list-data collection-list-tenet">{item.tenant_name}</td>
                  <td className="px-5 collection-list-data collection-list-data-building">{item.building_name}</td>
                  <td className="px-5 collection-list-data list-mob">{item.unit_name}</td>
                  <td className="px-5 collection-list-data list-mob">{parseFloat(item.total_amount ?? 0).toFixed(2)}</td>
                  <td className="px-5 collection-list-data list-mob">{item.end_date}</td>
                ...
                <AnimatePresence>
                  {activeDropdowns[item.id] && (
                    <motion.tr
                      className={`collection-dropdown-row ${activeDropdowns[item.id] ? 'mobile-with-border' : ''}`}
                      initial="hidden"
                      animate="visible"
                      exit="hidden"
                      ...
                      <td colSpan="4" className="pb-5">
                        <div className="grid !grid-cols-3 px-5">
                          ...
                          <div className='flex flex-col items-start'>
                            ...
                            <p className='drop-down-data'>{item.charge}</p>
                          </div>
                          <div className='flex flex-col items-start pl-4'>
                            <h4 className='drop-down-head'>TOTAL</h4>
                            <p className='drop-down-data'>{parseFloat(item.total_amount ?? 0).toFixed(2)}</p>
                          </div>
                        </div>
