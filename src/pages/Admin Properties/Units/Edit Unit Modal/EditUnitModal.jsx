...
const EditUnitModal = ({ open, onClose, title, unitId, onUnitCreated }) => {
  ...
  const steps = [
    ...
    { id: 2, title: "Update Unit", pageIndex: 1 },
          ...
          {isExpanded && (
            <div className="bg-white w-full mx-[16px] rounded-md absolute max-w-[calc(100vw-32px)] top-[-20px] shadow-[0_4px_6px_-1px_rgba(0,0,0,0.1)] z-20 overflow-hidden">
              {steps.map((step) => (
                <div
                  key={step.id}
                  className={`flex items-center space-x-5 p-3 cursor-pointer transition-colors duration-200 ${currentStep === step.id ? 'bg-[#1458A210]' : 'hover:bg-gray-50'
                    }`}
                  onClick={(e) => {
                    e.stopPropagation();
                ...
                >
                  <div className={`w-[30px] h-[30px] rounded-full flex items-center justify-center toggle-count ${currentStep === step.id ? 'bg-[#1458A2] text-white' : 'bg-[#1458A210] text-[#0a5ebf]'
                    }`}>
                    {step.id}
                  </div>
                  <span className={`mob-toggle-head text-[16px] ${currentStep === step.id ? 'text-[#1458A2] font-medium' : 'text-[#1458A2]'
                    }`}>
                    {step.title}
                  </span>
                </div>
