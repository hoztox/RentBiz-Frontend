...
const Expense = () => {
  ...
  const formatStatus = (status) => {
    if (!status) return "Pending"; // Explicit default
    return status.charAt(0).toUpperCase() + status.slice(1).toLowerCase(); // Capitalize first letter
  };
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${BASE_URL}/finance/expenses/company/${companyId}/`,
          {
            params: {
              search: searchTerm,
    ...
    fetchExpenses();
  }, [searchTerm, selectedOption, refreshCounter]);
      ...
      <div className="flex justify-between items-center p-5 border-b border-[#E9E9E9] expense-table-header">
        ...
        <div className="flex flex-col md:flex-row gap-[10px] expense-inputs-container">
          ...
          <div className="flex gap-[10px] expense-action-buttons w-full md:w-auto justify-start">
            ...
            <button
              className="flex items-center justify-center gap-2 h-[38px] rounded-md duration-200 expense-download-btn w-[122px]"
              onClick={() => toast.error("Download temporarily unavailable")}
            >
              Download
              <img
                src={downloadicon}
        ...
        <>
            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-[#E9E9E9] h-[57px]">
                  ...
                  <th className="px-5 text-left expense-thead w-[150px]">
                    TAX
                  </th>
                  <th className="px-5 text-left expense-thead w-[180px]">
                    TOTAL AMOUNT
                  </th>
                  <th className="px-5 text-left expense-thead w-[68px]">
                    STATUS
                  </th>
                  <th className="px-5 pr-11 text-right expense-thead w-[150px]">
                    ACTION
                  </th>
                </tr>
              </thead>
                ...
                {paginatedExpenses.map((expense, index) => (
                  ...
                  >
                    ...
                    <td className="px-5 text-left expense-data">
                      <span
                        className={`px-[10px] py-[5px] rounded-[4px] w-[69px] h-[28px] ${
                          formatStatus(expense.status) === "Paid"
                    ...
                    <td className="px-5 flex gap-[23px] items-center justify-end h-[57px]">
                      ...
                      <button
                        onClick={() =>
                          toast.error("Print temporarily unavailable")
                        }
                      >
                        <img
                          src={printericon}
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center py-2 md:px-5 expense-pagination-container">
            <span className="expense-pagination collection-list-pagination">
              Showing{" "}
              {Math.min((currentPage - 1) * itemsPerPage + 1, expenses.length)}{" "}
              to {Math.min(currentPage * itemsPerPage, expenses.length)} of{" "}
