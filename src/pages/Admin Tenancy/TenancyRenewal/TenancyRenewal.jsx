...
const TenancyRenewal = () => {
  ...
  return (
    <div className="border border-[#E9E9E9] rounded-md tenancy-table">
            ...
            {paginatedData.map((tenancy) => (
              ...
              >
                ...
                <td className="px-5 text-center !text-[#1458a2] tenancy-data">
                  <button onClick={() => handleRenewClick(tenancy)} className="trenew-renew-btn">
                    {tenancy.renew}
                  </button>
                </td>
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="tenancy-table-row-head">
              <th className="px-5 w-[74px] text-left tenancy-thead trenew-id-column">ID</th>
              <th className="px-3 text-left tenancy-thead trenew-end-date-column">END DATE</th>
          ...
          <tbody>
            {paginatedData.map((tenancy) => (
              <React.Fragment key={tenancy.id}>
                <tr
                  className={`${
                    expandedRows[tenancy.id] ? "trenew-mobile-no-border" : "trenew-mobile-with-border"
                  } border-b border-[#E9E9E9] h-[57px]`}
                >
                  <td className="px-5 text-left tenancy-data trenew-id-column">{tenancy.id}</td>
                  <td className="px-3 text-left tenancy-data trenew-end-date-column">{tenancy.endDate}</td>
                  ...
                  <tr className="trenew-mobile-with-border border-b border-[#E9E9E9]">
                    <td colSpan={3} className="px-5">
                      <div className="tenancy-dropdown-content mb-[16px]">
                        <div className="trenew-grid trenew-grid-cols-2">
                          <div className="trenew-grid-item">
                            <div className="trenew-dropdown-label">NAME</div>
                            <div className="trenew-dropdown-value">{tenancy.tenant}</div>
                          </div>
                          <div className="trenew-grid-item">
                            <div className="trenew-dropdown-label">BUILDING NAME</div>
                            <div className="trenew-dropdown-value">{tenancy.building}</div>
                          </div>
                        </div>
                        <div className="trenew-grid trenew-grid-cols-2">
                          <div className="trenew-grid-item">
                            <div className="trenew-dropdown-label">UNIT NAME</div>
                            <div className="trenew-dropdown-value">{tenancy.unit}</div>
                          </div>
                          <div className="trenew-grid-item">
                            <div className="trenew-dropdown-label">RENTAL MONTHS</div>
                            <div className="trenew-dropdown-value">{tenancy.months}</div>
                          </div>
                        </div>
                        <div className="trenew-grid trenew-grid-cols-3">
                          <div className="trenew-grid-item">
                          ...
                          <div className="trenew-grid-item trenew-view-column">
                          ...
                          <div className="trenew-grid-item trenew-action-column">
                            <div className="trenew-dropdown-label">ACTION</div>
                            <div className="trenew-dropdown-value trenew-flex trenew-items-center trenew-gap-2 mt-[10px]">
                              <button onClick={openTenancyUpdateModal}>
                                <img
                                  src={editicon}
                                  alt="Edit"
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-3 md:px-5 trenew-pagination-container">
        <span className="trenew-collection-list-pagination trenew-pagination-text">
          Showing {Math.min((currentPage - 1) * itemsPerPage + 1, filteredData.length)} to{" "}
          {Math.min(currentPage * itemsPerPage, filteredData.length)} of {filteredData.length} entries
        </span>
        <div className="flex gap-[4px] overflow-x-auto py-2 w-full md:w-auto trenew-pagination-buttons">
          <button
            className="px-[10px] py-[6px] rounded-md bg-[#F4F4F4] hover:bg-[#e6e6e6] duration-200 cursor-pointer trenew-pagination-btn"
          ...
          {startPage > 1 && (
            <button
              className="px-4 h-[38px] rounded-md cursor-pointer duration-200 trenew-page-no-btns bg-[#F4F4F4] hover:bg-[#e6e6e6] text-[#677487]"
          ...
          {[...Array(endPage - startPage + 1)].map((_, i) => (
            <button
              key={startPage + i}
              className={`px-4 h-[38px] rounded-md cursor-pointer duration-200 trenew-page-no-btns ${
